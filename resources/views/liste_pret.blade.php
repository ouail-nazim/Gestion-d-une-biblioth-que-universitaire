    ...
    <div class="container">
        <table class="table table-striped " id="myTable">
            ...
            @foreach($abonner as $abo)
                @if(($abo->emprunt )!= null)
                    @foreach($abo->emprunt as $emprunt)
                            <tr class="filterDiv @if ($emprunt->date_retour < \Carbon\Carbon::today()->toDateString() )retarde  @endif">
                                <td>{{$abo->nom}}</td>
                                <td>{{$abo->prenom}}</td>
                                <td>{{$abo->email}}</td>
                                <td>0{{$abo->telephone}}</td>
                                <td>{{$emprunt->exemplaire->document->titre}}</td>
                                <td>{{$emprunt->date_retour}}</td>
                                {{--<td><a href="/pinaliser/{{$abo->id}}" class="btn btn-warning">Penaliser</a></td>--}}
                            </tr>
